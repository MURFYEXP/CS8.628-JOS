...
// boot_alloc暂时当做页分配器，之后使用的真实页分配器是page_alloc
static void *
boot_alloc(uint32_t n)
{
    // 函数的核心维护一个静态变量nextfree，里面存放着下一个可以使用的空闲内存空间的虚拟地址
    ...
    // ROUNDUP保证nextfree与页面大小PGSIZE对齐
    // 分配n个字节的内存时，需要修改nextfree的值
...
page_init(void)
{
    ...
    // 初始化pages数组
    // 第0页已被占用,io hole部分已被占用，在extmem区域有一部分已经被占用
	size_t i;
    page_free_list = NULL;
    
    //num_alloc：在extmem区域已经被占用的页的个数, KERNBASE 0xf0000000内核基址
    ...
    for(i = 0; i < npages; i++)
    {
        ...
        // so that we can safely copy and run AP bootstrap code at that physical address
...
// 分配一个物理页,返回这个物理页所对应的PageInfo
// 从free_page_list中取出一个空闲页的PageInfo
// 修改free_page_list相关信息
// 修改取出的空闲页的PageInfo，初始化该页的内存
struct PageInfo *
...
// create 标志是0.仅仅是查询va地址所属的页是否存在. 存在就返回对
// 应的page table的入口地址，不存在就返回NULL
pte_t *
...
// 把虚拟地址空间范围[va, va+size)映射到物理空间[pa, pa+size)的映射关系加入到页表pgdir中
static void
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    int nadd;
    pte_t *entry = NULL;
    // 把虚拟页和物理页的映射关系存放到响应的页表项中,直到把size个字节的内存都分配完
...
// 如果va所在的虚拟内存页存在，那么取消当前va的虚拟内存页也和之前物理页的关联，
// 并且为va建立新的物理页联系——pp所描述的物理页
int
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
    ...
    pp->pp_ref++; //在page_remove之前执行，防止pp被释放
...
//
// 取消va对应物理页之间的关联，相当于刷新TLB，每次我们调整虚拟页和物理页
// 之间的映射关系的时候，我们都要刷新TLB
...
//
// user_mem_check 函数的功能是检查一下当前用户态程序是否有对虚拟地址空间 [va, va+len]
// 的 perm| PTE_P 访问权限
// 找到这个虚拟地址范围对应于当前用户态程序的页表中的页表项，看这个页表项中有关访问权限的字段，
// 是否包含 perm | PTE_P，只要有一个页表项是不包含的，就代表程序对这个范围的虚拟地址没有
// perm|PTE_P 的访问权限
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for(; start < end; start += PGSIZE)
    {
        ...
        if((int)start > ULIM || cur == NULL || ((uint32_t)(*cur) & perm) != perm)
        {
            ...
            // the first erroneous virtual address
...
// and page_init()).
// 检查page_alloc()，page_free()，page_init()函数是否能够正确运行
static void
